...
class StudentService:
    ...
    def apply_filters(
        ...
        if min_percentage is not None:
            query = query.filter(Student.percentage >= min_percentage)

        if max_percentage is not None:
            query = query.filter(Student.percentage <= max_percentage)

        if failed is True:
            query = query.filter(Student.grade == "F")
    ...
    def add_student(self, student_id: str, name: str, marks: Dict[str, float]):
        if self.db.query(Student).filter_by(student_id=student_id).first():
        ...
        try:
            ...
            if marks:
                existing_subjects = {
                    ...
                    .filter(Subject.name.in_(marks.keys()))
                    .all()
                }
    ...
    def get_student(self, student_id: str):
        student = (
            ...
            .options(joinedload(Student.marks).joinedload(StudentMarks.subject))
    ...
    def get_students(
        ...
        """
        Fetch paginated students with filters applied at DB level.
        Ensures efficient querying and scalability.
        """

        query = self.db.query(Student).options(
            joinedload(Student.marks).joinedload(StudentMarks.subject)
        )

        query = self.apply_filters(
            query, name, grade, min_percentage, max_percentage, failed
        )
    ...
    def update_student(self, student_id: str, data: UpdateMarksSchema):
        ...
        try:
            ...
            if data.marks:
                self.db.query(StudentMarks).filter_by(student_id=student_id).delete()

                existing_subjects = {}
                if data.marks:
                    existing_subjects = {
                        s.name: s for s in self.db.query(Subject)
                        .filter(Subject.name.in_(data.marks.keys())).all()
                    }
                ...
                total, percentage, grade = self.calculate(data.marks)
    ...
    def get_topper(self):
        student = self.db.query(Student).order_by(Student.percentage.desc()).first()
    ...
    def get_ranked_list(self):
        return self.db.query(Student).order_by(
            Student.percentage.desc(), Student.name.asc()
        ).all()
    ...
    def get_subject_analysis(self):
        results = (
            self.db.query(Subject.name, StudentMarks.marks)
    ...
    def dashboard(self):
        ...
        passed = sum(1 for s in students if s.grade != "F")
        ...
        return {
            ...
            "grades": grades,
            "average_percentage": self.get_average_percentage(),
            "topper": topper.name
        }
    ...
    def format_student(self, student):
        return {
            ...
            "grade": student.grade,
            "marks": [
                {"subject": m.subject.name, "marks": m.marks}
